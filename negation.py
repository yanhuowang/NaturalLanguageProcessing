...
def main():
	...
	training_set_n = documents_n[:1600] #1600 sentences in training set
	held_out_set_n = documents_n[-400:-200] #200 sentences in held out set
	test_set_n = documents_n[-200:] # 200 sentences in test set

	sentiment_n = Sentiment()
	sentiment_n.Set_word_stat(training_set_n)
	sentiment_n.Conclude_word_stat()
	real_labels_n = sentiment_n.Real_labels(held_out_set_n)

	sentiment_n.Uni_count_list()
	sentiment_n.Uni_high_score_list()
	...
	for cutoff in cutoffs:
		...
		trainsets_n = sentiment_n.Feature_set_uni(cutoff, training_set_n, 2)
		held_out_sets_n = sentiment_n.Feature_set_uni(cutoff, held_out_set_n, 2)
		algorithm = nltk.classify.MaxentClassifier.ALGORITHMS[0]
		classifier_n = nltk.MaxentClassifier.train(trainsets_n, algorithm, max_iter = 50)
		classifier_n.show_most_informative_features(10)
		print "\nClassifier Accuracy : %4f\n" % nltk.classify.accuracy(classifier_n, held_out_sets_n)
		predicted_labels_n = sentiment_n.Maxent_predicted(classifier_n, held_out_sets_n)
		sentiment_n.Evaluation(predicted_labels_n, real_labels_n)

	# without info gain
	print "--- without information gain---"

	for cutoff in cutoffs:
		print "cutoff = " + str(cutoff)
		trainsets_n = sentiment_n.Feature_set_uni(cutoff, training_set_n, 1)
		held_out_sets_n = sentiment_n.Feature_set_uni(cutoff, held_out_set_n, 1)
		algorithm = nltk.classify.MaxentClassifier.ALGORITHMS[0]
		classifier_n = nltk.MaxentClassifier.train(trainsets_n, algorithm, max_iter = 50)
		classifier_n.show_most_informative_features(10)
		print "\nClassifier Accuracy : %4f\n" % nltk.classify.accuracy(classifier_n, held_out_sets_n)
